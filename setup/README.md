...
- `databricks/generate_tvendor_env.py`: generates `config/tvendor.env` (and normalizes `app/app.yaml`) from a single app config source for Databricks OAuth runtime (no PAT token).
