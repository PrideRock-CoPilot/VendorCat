...
def test_create_vendor_writes_canonical_vendor_tables(client: TestClient, isolated_local_db: Path) -> None:
    response = client.post(
        "/vendors/new",
        data={
            "return_to": "/vendors",
            "legal_name": "Canonical Vendor LLC",
            "display_name": "Canonical Vendor",
            "lifecycle_state": "draft",
            "owner_org_choice": "IT-ENT",
            "risk_tier": "low",
            "source_system": "manual",
        },
        follow_redirects=False,
    ...
    assert response.status_code == 303
    match = re.search(r"/vendors/(vnd-[^/]+)/summary", response.headers.get("location", ""))
    assert match is not None
    vendor_id = match.group(1)

    with sqlite3.connect(str(isolated_local_db)) as conn:
        core_row = conn.execute(
            "SELECT vendor_id FROM core_vendor WHERE vendor_id = ?",
            (vendor_id,),
        ).fetchone()
        canonical_row = conn.execute(
            (
                "SELECT vendor_id, legal_name, display_name, lifecycle_state_id, "
                "risk_tier_id, primary_owner_organization_id "
        ...
        assert str(canonical_row[1]) == "Canonical Vendor LLC"
        assert str(canonical_row[2]) == "Canonical Vendor"
        assert str(canonical_row[3]).strip() != ""
        assert str(canonical_row[4]).strip() != ""
        assert str(canonical_row[5]).strip() != ""


def test_create_offering_writes_canonical_offering_and_bu_assignment(
    client: TestClient,
    ...
    response = client.post(
        ...
        data={
            "return_to": "/vendors",
            "offering_name": "Canonical Offering",
            "offering_type": "SaaS",
    ...
    assert response.status_code == 303
    match = re.search(r"/vendors/vnd-003/offerings/(off-[^?]+)", response.headers.get("location", ""))
    assert match is not None
    offering_id = match.group(1)

    with sqlite3.connect(str(isolated_local_db)) as conn:
        canonical_offering = conn.execute(
            (
                "SELECT offering_id, vendor_id, offering_name, lifecycle_state_id, "
                "primary_business_unit_id, primary_service_type_id "
                "FROM offering WHERE offering_id = ?"
            ),
            (offering_id,),
        ).fetchone()
        canonical_assignment = conn.execute(
            (
                "SELECT assignment_id, business_unit_id, is_primary, active_flag "
                "FROM offering_business_unit_assignment "
                "WHERE offering_id = ? AND active_flag = 1"
            ),
            (offering_id,),
        ).fetchone()
        assert canonical_offering is not None
        assert str(canonical_offering[1]) == "vnd-003"
        assert str(canonical_offering[2]) == "Canonical Offering"
        assert str(canonical_offering[3]).strip() != ""
        assert str(canonical_offering[4]).strip() != ""
        assert str(canonical_offering[5]).strip() != ""
        assert canonical_assignment is not None
        assert int(canonical_assignment[2] or 0) == 1
        assert int(canonical_assignment[3] or 0) == 1


def test_apply_vendor_profile_update_writes_canonical_business_unit_crosswalk(
    client: TestClient,
    isolated_local_db: Path,
) -> None:
    repo = get_repo()
    result = repo.apply_vendor_profile_update(
        ...
        updates={"owner_org_id": "Finance", "business_unit_ids": ["Finance", "IT"]},
        reason="canonical bu crosswalk test",
    )
    assert str(result.get("request_id") or "").strip() != ""

    with sqlite3.connect(str(isolated_local_db)) as conn:
        vendor_row = conn.execute(
            "SELECT primary_business_unit_id FROM vendor WHERE vendor_id = ?",
            ("vnd-001",),
        ).fetchone()
        active_assignments = conn.execute(
            (
                "SELECT business_unit_id, is_primary, active_flag "
                "FROM vendor_business_unit_assignment "
                "WHERE vendor_id = ? AND active_flag = 1"
            ),
            ("vnd-001",),
        ).fetchall()
        assert vendor_row is not None
        assert str(vendor_row[0] or "").strip() != ""
        assignment_ids = {str(row[0] or "").strip() for row in active_assignments}
        assert len(assignment_ids) >= 2
        assert any(int(row[1] or 0) == 1 for row in active_assignments)
        assert all(int(row[2] or 0) == 1 for row in active_assignments)


def test_execute_vendor_merge_syncs_canonical_vendor_and_offering_writes(
    client: TestClient,
    isolated_local_db: Path,
) -> None:
    repo = get_repo()
    with sqlite3.connect(str(isolated_local_db)) as conn:
        conn.execute(
            "UPDATE core_vendor_offering SET offering_name = ? WHERE offering_id = ?",
            ("Azure", "off-003"),
        )
        conn.commit()

    preview = repo.preview_vendor_merge(
        survivor_vendor_id="vnd-001",
        source_vendor_id="vnd-002",
    )
    collisions = list(preview.get("offering_collisions") or [])
    assert any(str(item.get("source_offering_id") or "") == "off-003" for item in collisions)
    field_decisions = {
        str(item.get("field_name") or "").strip(): "survivor"
    ...
    result = repo.execute_vendor_merge(
        ...
        offering_decisions={
            "off-003": {
                "decision": "merge",
                "target_offering_id": "off-002",
            }
        },
        actor_user_principal="admin@example.com",
        merge_reason="canonical runtime write merge sync test",
    )
    assert str(result.get("merge_id") or "").strip() != ""
    assert int((result.get("offering_summary") or {}).get("merged_into_existing") or 0) >= 1

    with sqlite3.connect(str(isolated_local_db)) as conn:
        source_vendor_core = conn.execute(
            "SELECT lifecycle_state, merged_into_vendor_id FROM core_vendor WHERE vendor_id = ?",
            ("vnd-002",),
        ).fetchone()
        assert source_vendor_core is not None
        assert str(source_vendor_core[0]) == "inactive"
        assert str(source_vendor_core[1]) == "vnd-001"

        source_vendor_canonical = conn.execute(
            (
                "SELECT l.lifecycle_state_code "
                "FROM vendor v "
                "INNER JOIN lkp_lifecycle_state l ON l.lifecycle_state_id = v.lifecycle_state_id "
                "WHERE v.vendor_id = ?"
            ),
            ("vnd-002",),
        ).fetchone()
        assert source_vendor_canonical is not None
        assert str(source_vendor_canonical[0]) == "inactive"

        source_offering_canonical = conn.execute(
            (
                "SELECT o.vendor_id, o.offering_name, l.lifecycle_state_code "
                "FROM offering o "
                "INNER JOIN lkp_lifecycle_state l ON l.lifecycle_state_id = o.lifecycle_state_id "
                "WHERE o.offering_id = ?"
            ),
            ("off-003",),
        ).fetchone()
        assert source_offering_canonical is not None
        assert str(source_offering_canonical[0]) == "vnd-001"
        assert "Merged into off-002" in str(source_offering_canonical[1] or "")
        assert str(source_offering_canonical[2]) == "retired"

        moved_vendor_owner = conn.execute(
            "SELECT vendor_id FROM vendor_owner_assignment WHERE assignment_id = ?",
            ("vown-003",),
        ).fetchone()
        moved_vendor_contact = conn.execute(
            "SELECT vendor_id FROM vendor_contact WHERE vendor_contact_id = ?",
            ("con-002",),
        ).fetchone()
        moved_vendor_bu = conn.execute(
            "SELECT vendor_id FROM vendor_business_unit_assignment WHERE assignment_id = ?",
            ("voa-003",),
        ).fetchone()
        moved_offering_owner = conn.execute(
            "SELECT offering_id FROM offering_owner_assignment WHERE assignment_id = ?",
            ("oown-003",),
        ).fetchone()
        moved_offering_contact = conn.execute(
            "SELECT offering_id FROM offering_contact WHERE offering_contact_id = ?",
            ("ocon-003",),
        ).fetchone()

        assert moved_vendor_owner is not None
        assert moved_vendor_contact is not None
        assert moved_vendor_bu is not None
        assert moved_offering_owner is not None
        assert moved_offering_contact is not None
        assert str(moved_vendor_owner[0]) == "vnd-001"
        assert str(moved_vendor_contact[0]) == "vnd-001"
        assert str(moved_vendor_bu[0]) == "vnd-001"
        assert str(moved_offering_owner[0]) == "off-002"
        assert str(moved_offering_contact[0]) == "off-002"
...
def test_create_vendor_validation_keeps_values_and_marks_owner_org(client: TestClient) -> None:
    ...
    assert "Business Unit is required." in response.text
...
def test_offering_type_uses_dropdown_in_new_and_edit_forms(client: TestClient) -> None:
    ...
    assert '<select name="business_unit">' in new_form.text
    ...
    assert '<select name="business_unit">' in edit_form.text
...
def test_vendor_summary_includes_active_lob_and_service_type_values(client: TestClient) -> None:
    ...
    assert response.status_code == 200
    assert "active_business_units" in response.text
...
def test_vendor_ownership_allows_adding_owner_lob_and_contact(client: TestClient) -> None:
    ...
    lob_response = client.post(
        "/vendors/vnd-002/ownership/business_unit/update",
        data={
            "return_to": "/vendors/vnd-002/ownership",
            "business_unit": "Finance",
            "reason": "Set single business unit org value.",
        },
        follow_redirects=False,
    )
    ...
    assert "Business Unit" in ownership_page.text
    ...
    assert "vendor-business_unit-update-template" in ownership_page.text
    assert 'type="checkbox"' in ownership_page.text
    assert 'name="primary_business_unit"' in ownership_page.text
    assert 'name="business_units"' in ownership_page.text
...
def test_vendor_ownership_lob_update_supports_explicit_primary_lob(client: TestClient) -> None:
    response = client.post(
        ...
        data={
            "return_to": "/vendors/vnd-002/ownership",
            "business_units": ["Finance", "Security"],
            "primary_business_unit": "Security",
...
def test_vendor_ownership_lobs_render_primary_then_alphabetical(client: TestClient) -> None:
    response = client.post(
        ...
        data={
            "return_to": "/vendors/vnd-002/ownership",
            "business_units": ["Security", "Finance", "Enterprise"],
            "primary_business_unit": "Security",
            "reason_code": "ownership_alignment",
        },
        follow_redirects=False,
...
def test_vendor_ownership_drawer_prepopulates_active_lobs_and_bulk_controls(client: TestClient) -> None:
    ...
    ownership_page = client.get("/vendors/vnd-002/ownership?return_to=%2Fvendors")
    assert ownership_page.status_code == 200
    assert "data-vendor-select-all-business_units" in ownership_page.text
    assert "data-vendor-clear-all-business_units" in ownership_page.text
    ...
    rendered_lob_values = set(
        re.findall(r'name="business_units"[^>]*value="([^"]+)"', ownership_page.text, flags=re.IGNORECASE)
    )
    ...
    for lob_value in active_lobs:
        ...
        checked_pattern = re.compile(
            rf'name="business_units"[^>]*value="{re.escape(lob_value)}"[^>]*checked|value="{re.escape(lob_value)}"[^>]*name="business_units"[^>]*checked',
            flags=re.IGNORECASE,
        )
...
def test_offering_ownership_org_updates_single_lob_value(client: TestClient) -> None:
    response = client.post(
        "/vendors/vnd-001/offerings/off-004/ownership/business_unit/update",
        data={
            "return_to": "/vendors/vnd-001/offerings/off-004?section=ownership&return_to=%2Fvendors",
    ...
    page = client.get("/vendors/vnd-001/offerings/off-004?section=ownership&return_to=%2Fvendors")
    assert page.status_code == 200
    assert "Business Unit" in page.text
    assert "Finance" in page.text
    assert 'name="business_units"' in page.text
    assert 'name="primary_business_unit"' in page.text
    assert 'type="checkbox"' in page.text
    assert 'name="reason_code"' in page.text
    assert "data-offering-select-all-business_units" in page.text
    assert "data-offering-clear-all-business_units" in page.text
...
def test_offering_ownership_lob_update_supports_explicit_primary_lob(client: TestClient) -> None:
    ...
    assert str(offering.get("business_unit") or "").strip() == "Security"
...
def test_offering_ownership_lob_update_allows_no_primary_lob(client: TestClient) -> None:
    ...
    offering = get_repo().get_offering_record("vnd-001", "off-004")
    assert offering is not None
    assert str(offering.get("business_unit") or "").strip() == ""
