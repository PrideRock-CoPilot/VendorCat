...
def test_parse_layout_rows_supplier_aliases_map_to_vendor_fields() -> None:
    raw_bytes = (
        b"Supplier_Name,Supplier_ID,Owner_Org_ID,Lifecycle_State\n"
        b"Acme Supplier LLC,SUP-001,IT,draft\n"
    )
    parsed = parse_layout_rows(
        "vendors",
        raw_bytes,
    ...
    rows = list(parsed.get("rows") or [])
    assert len(rows) == 1
    assert rows[0]["legal_name"] == "Acme Supplier LLC"
    assert rows[0]["vendor_id"] == "SUP-001"
    assert rows[0]["owner_org_id"] == "IT"


def test_parse_layout_rows_flat_repeating_contacts_emit_multiple_stage_rows() -> None:
    raw_bytes = (
        b"legal_name,owner_org_id,contact_1_name,contact_1_email,contact_2_name,contact_2_email\n"
        b"Grouped Vendor,IT,A One,a.one@example.com,B Two,b.two@example.com\n"
    )
    parsed = parse_layout_rows(
        "vendors",
        raw_bytes,
        ...
        source_target_mapping={
            ...
            "contact_1_name": "vendor_contact.full_name",
            "contact_1_email": "vendor_contact.email",
            "contact_2_name": "vendor_contact.full_name",
            "contact_2_email": "vendor_contact.email",
        },
    )
    stage_rows = dict(parsed.get("stage_area_rows") or {})
    contact_rows = list(stage_rows.get("vendor_contact") or [])
    ...
    assert names == ["A One", "B Two"]
    group_keys = sorted([str(row.get("source_group_key") or "") for row in contact_rows])
    assert group_keys == ["contact_1", "contact_2"]
