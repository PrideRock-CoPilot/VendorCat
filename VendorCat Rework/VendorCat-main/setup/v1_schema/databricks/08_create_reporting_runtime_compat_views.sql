...
CREATE OR REPLACE VIEW vw_reporting_core_vendor AS
SELECT
  v.vendor_id,
  v.legal_name,
  v.display_name,
  ...
  CAST(NULL AS STRING) AS source_batch_id,
  CAST(NULL AS STRING) AS source_extract_ts,
  CAST(NULL AS STRING) AS merged_into_vendor_id,
  CAST(NULL AS STRING) AS merged_at,
  CAST(NULL AS STRING) AS merged_by,
  CAST(NULL AS STRING) AS merge_reason,
  ...
  v.delegated_vendor_flag AS delegated_vendor_flag,
  v.health_care_vendor_flag AS health_care_vendor_flag,
  CAST(v.created_at AS STRING) AS created_at,
  CAST(v.updated_at AS STRING) AS updated_at,
  v.updated_by
FROM vendor v
...
SELECT
  ...
  cv.merged_by,
  cv.merge_reason,
  CAST(NULL AS STRING) AS vendor_category,
  CAST(NULL AS STRING) AS compliance_category,
  CAST(NULL AS STRING) AS gl_category,
  CAST(NULL AS BOOLEAN) AS delegated_vendor_flag,
  CAST(NULL AS BOOLEAN) AS health_care_vendor_flag,
...
CREATE OR REPLACE VIEW vw_reporting_core_vendor_offering AS
SELECT
  o.offering_id,
  o.vendor_id,
  o.offering_name,
  CAST(NULL AS STRING) AS offering_type,
  ...
  CAST(o.updated_at AS STRING) AS updated_at,
  o.updated_by
FROM offering o
LEFT JOIN lkp_business_unit bu
...
CREATE OR REPLACE VIEW vw_reporting_core_vendor_business_owner AS
SELECT
  ...
  voa.active_flag AS active_flag,
  CAST(voa.created_at AS STRING) AS created_at,
  CAST(coalesce(voa.ended_at, voa.created_at) AS STRING) AS updated_at,
...
SELECT
  ...
  cbo.owner_role,
  cbo.active_flag = 1 AS active_flag,
  CAST(NULL AS STRING) AS created_at,
  cbo.updated_at,
  cbo.updated_by
...
CREATE OR REPLACE VIEW vw_reporting_core_offering_business_owner AS
SELECT
  ...
  ooa.active_flag AS active_flag,
  CAST(ooa.created_at AS STRING) AS created_at,
  CAST(coalesce(ooa.ended_at, ooa.created_at) AS STRING) AS updated_at,
  CAST(NULL AS STRING) AS updated_by
...
CREATE OR REPLACE VIEW vw_reporting_core_vendor_contact AS
SELECT
  ...
  vc.phone,
  vc.active_flag,
  CAST(vc.created_at AS STRING) AS created_at,
  CAST(coalesce(vc.ended_at, vc.created_at) AS STRING) AS updated_at,
  CAST(NULL AS STRING) AS updated_by
...
SELECT
  ...
  cvc.email,
  cvc.phone,
  cvc.active_flag = 1 AS active_flag,
  CAST(NULL AS STRING) AS created_at,
  cvc.updated_at,
  cvc.updated_by
...
CREATE OR REPLACE VIEW vw_reporting_core_offering_contact AS
SELECT
  ...
  oc.phone,
  oc.active_flag,
  CAST(oc.created_at AS STRING) AS created_at,
  CAST(coalesce(oc.ended_at, oc.created_at) AS STRING) AS updated_at,
  CAST(NULL AS STRING) AS updated_by
...
SELECT
  ...
  coc.email,
  coc.phone,
  coc.active_flag = 1 AS active_flag,
  CAST(NULL AS STRING) AS created_at,
  coc.updated_at,
  coc.updated_by
...
CREATE OR REPLACE VIEW vw_reporting_core_vendor_identifier AS
SELECT
  vi.vendor_identifier_id,
  vi.vendor_id,
  vi.identifier_type,
  vi.source_vendor_key AS identifier_value,
  vi.is_primary_source AS is_primary,
  CAST(NULL AS STRING) AS country_code,
  CAST(vi.updated_at AS STRING) AS updated_at,
  CAST(NULL AS STRING) AS updated_by
...
SELECT
  ...
  cvi.is_primary = 1 AS is_primary,
  cvi.country_code,
  cvi.updated_at,
  cvi.updated_by
...
CREATE OR REPLACE VIEW vw_reporting_core_vendor_org_assignment AS
SELECT
  vba.assignment_id AS vendor_org_assignment_id,
  vba.vendor_id,
  coalesce(bu.business_unit_name, vba.business_unit_id) AS org_id,
  CASE
    WHEN coalesce(vba.is_primary, false) THEN 'primary'
    ELSE 'consumer'
  END AS assignment_type,
  coalesce(vba.active_flag, true) AS active_flag,
  CAST(vba.created_at AS STRING) AS created_at,
  CAST(coalesce(vba.updated_at, vba.created_at) AS STRING) AS updated_at,
  vba.updated_by
...
SELECT
  ...
  cvoa.active_flag = 1 AS active_flag,
  CAST(NULL AS STRING) AS created_at,
  cvoa.updated_at,
  cvoa.updated_by
...
CREATE OR REPLACE VIEW vw_reporting_core_contract AS
SELECT
  c.contract_id,
  c.vendor_id,
  c.offering_id,
  ...
  c.start_date,
  c.end_date,
  coalesce(c.cancelled_flag, false) AS cancelled_flag,
  c.annual_value,
  CAST(c.updated_at AS STRING) AS updated_at,
  c.updated_by
FROM contract c
UNION ALL
SELECT
  ...
  cc.start_date,
  cc.end_date,
  cc.cancelled_flag = 1 AS cancelled_flag,
  cc.annual_value,
  cc.updated_at,
  cc.updated_by
...
CREATE OR REPLACE VIEW vw_reporting_core_contract_event AS
SELECT
...
CREATE OR REPLACE VIEW vw_reporting_core_vendor_demo AS
SELECT
  d.demo_id,
  d.vendor_id,
  d.offering_id,
  ...
  d.notes,
  CAST(d.updated_at AS STRING) AS updated_at,
  d.updated_by
FROM vendor_demo d
UNION ALL
SELECT
...
CREATE OR REPLACE VIEW vw_reporting_core_vendor_demo_score AS
SELECT
  ds.demo_score_id,
  ds.demo_id,
  ds.score_category,
...
CREATE OR REPLACE VIEW vw_reporting_core_vendor_demo_note AS
SELECT
  dn.demo_note_id,
  dn.demo_id,
  dn.note_type,
  dn.note_text,
  CAST(dn.created_at AS STRING) AS created_at,
  dn.created_by
