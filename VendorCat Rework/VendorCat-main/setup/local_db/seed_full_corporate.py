...
def seed_full_corporate(conn: sqlite3.Connection, config: SeedConfig | None = None) -> dict[str, int]:
    ...
    for idx in range(1, cfg.vendor_count + 1):
        ...
        for off_idx in range(1, offerings_for_vendor + 1):
            ...
            business_unit = lob_values[(idx + (off_idx * 2)) % len(lob_values)]
    ...
    conn.executemany(
        ...
        INSERT INTO core_vendor_offering
          (offering_id, vendor_id, offering_name, offering_type, business_unit, service_type, lifecycle_state, criticality_tier, updated_at, updated_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        offering_rows,
    )
