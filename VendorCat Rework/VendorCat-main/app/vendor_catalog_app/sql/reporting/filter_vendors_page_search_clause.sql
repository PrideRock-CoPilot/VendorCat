...
lower(v.vendor_id) LIKE lower(%s)
 ...
 OR EXISTS (
  SELECT 1 FROM {core_vendor_offering} o
 ...
 OR EXISTS (
  SELECT 1 FROM {core_vendor_business_owner} bo
 ...
 OR EXISTS (
  SELECT 1 FROM {core_offering_business_owner} obo
  INNER JOIN {core_vendor_offering} o2 ON obo.offering_id = o2.offering_id
 ...
 OR EXISTS (
  SELECT 1 FROM {core_vendor_contact} vc
 ...
 OR EXISTS (
  SELECT 1 FROM {core_offering_contact} oc
  INNER JOIN {core_vendor_offering} o3 ON oc.offering_id = o3.offering_id
 ...
 OR EXISTS (
  SELECT 1 FROM {core_vendor_demo} d
