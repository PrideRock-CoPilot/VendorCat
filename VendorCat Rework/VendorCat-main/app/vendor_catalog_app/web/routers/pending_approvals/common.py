...
def _payload_business_unit_values(payload: dict[str, Any], offering_business_unit_by_id: dict[str, str]) -> list[str]:
    business_units: list[str] = []
    for key in ("business_unit", "line_of_business"):
        value = str(payload.get(key, "")).strip()
        if value:
            business_units.append(value)
    updates = payload.get("updates")
    if isinstance(updates, dict):
        value = str(updates.get("business_unit", "")).strip()
        if value:
            business_units.append(value)
    for offering_id in _payload_offering_ids(payload):
        business_unit = str(offering_business_unit_by_id.get(offering_id, "")).strip()
        if business_unit:
...
def _load_workflow_queue_view(
    ...
    *,
    ...
    offering_business_unit_by_id: dict[str, str] = {}
    if offering_ids:
        ...
        for offering in offering_rows:
            ...
            if not offering_id:
                continue
            offering_business_unit_by_id[offering_id] = str(offering.get("business_unit") or "").strip()
    ...
    business_unit_options_set: set[str] = set()
    ...
    for row in rows:
        ...
        row["_business_unit_values"] = _payload_business_unit_values(payload, offering_business_unit_by_id)
        row["_business_unit_display"] = ", ".join(row["_business_unit_values"]) if row["_business_unit_values"] else "-"
        ...
        if row["_business_unit_values"]:
            business_unit_options_set.update(row["_business_unit_values"])
        if requestor:
        ...
        if selected_queue == "my_approvals" and user.can_approve_requests:
            if not _row_in_user_scope(
                ...
                continue
        if selected_business_unit and selected_business_unit != "all":
            if selected_business_unit.lower() not in {str(item).lower() for item in row["_business_unit_values"]}:
                continue
    ...
    return {
        "rows": filtered_rows,
        "business_unit_options": sorted(business_unit_options_set, key=lambda item: item.lower()),
