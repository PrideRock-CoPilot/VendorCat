...
def admin_access(request: Request):
    ...
    selected_business_unit_filter = str(request.query_params.get("business_unit_filter") or "all").strip()
    if not selected_business_unit_filter:
        selected_business_unit_filter = "all"
    selected_tab = str(request.query_params.get("tab") or "users").strip().lower()
    if selected_tab not in {"users", "groups", "business_unit", "roles"}:
        selected_tab = "users"
    ...
    scope_base_params = {
        "tab": selected_tab,
        "business_unit_filter": selected_business_unit_filter,
        "page_size": str(page_size),
    ...
    scope_org_filter = "" if selected_business_unit_filter.lower() == "all" else selected_business_unit_filter
    ...
    business_unit_options = [
        str(item).strip()
        for item in list(repo.available_orgs() or [])
    ...
    return _render(
        ...
        "Admin Access",
        {
            ...
            "selected_business_unit_filter": selected_business_unit_filter,
            "page_size": page_size,
            "role_rows": role_rows,
    ...
    return _render(
        ...
        "Admin User Access",
        {
            ...
            "effective_scope_summary": (
                ", ".join(sorted(set(active_scopes))) if active_scopes else "No active Business Unit access grants"
            ),
        },
    )
