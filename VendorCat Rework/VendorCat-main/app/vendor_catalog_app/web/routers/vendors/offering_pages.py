...
from vendor_catalog_app.web.routers.vendors.pages import (
    ...
    _normalize_offering_business_unit,
...
def offering_new_form(request: Request, vendor_id: str, return_to: str = VENDOR_DEFAULT_RETURN_TO):
    ...
    context = base_template_context(
        ...
        extra={
            ...
            "offering_business_unit_options": _offering_business_unit_options(repo),
...
async def offering_new_submit(request: Request, vendor_id: str):
    ...
    try:
        ...
        business_unit = _normalize_offering_business_unit(repo, business_unit)
...
async def offering_update_submit(request: Request, vendor_id: str, offering_id: str):
    ...
    try:
        ...
        existing_business_unit = str(current_offering.get("business_unit") or "").strip()
        updates["business_unit"] = _normalize_offering_business_unit(
            repo,
            updates["business_unit"],
            extra_allowed={existing_business_unit} if existing_business_unit else None,
        )
        updates["business_unit"] = updates["business_unit"] or None
...
@router.post("/{vendor_id}/offerings/{offering_id}/ownership/business_unit/update")
@require_permission("offering_edit")
async def offering_ownership_business_unit_update_submit(request: Request, vendor_id: str, offering_id: str):
    repo = get_repo()
    user = get_user_context(request)
    form = await request.form()
    ...
    try:
        ...
        if not normalized_business_units:
            raise ValueError("Business Unit must be selected from the admin-managed list.")
        normalized_business_unit = _normalize_offering_business_unit(
            repo,
            primary_business_unit,
            allow_blank=True,
            extra_allowed={existing_business_unit} if existing_business_unit else None,
        )
        if normalized_business_unit and normalized_business_unit not in normalized_business_units:
            raise ValueError("Primary business unit must be one of the selected business units.")

        payload_updates = {"business_unit": normalized_business_unit}
        if user.can_apply_change("update_offering"):
            result = repo.update_offering_fields(
                vendor_id=vendor_id,
                offering_id=offering_id,
        ...
        repo.log_usage_event(
            user_principal=user.user_principal,
            page_name="vendor_offering_detail",
            event_type="offering_ownership_business_unit_update",
            payload={
                "vendor_id": vendor_id,
                "offering_id": offering_id,
