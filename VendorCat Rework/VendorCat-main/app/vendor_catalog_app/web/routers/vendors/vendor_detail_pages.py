...
from vendor_catalog_app.web.routers.vendors.pages import (
    ...
    _offering_business_unit_options,
...
def vendor_summary_page(request: Request, vendor_id: str, return_to: str = VENDOR_DEFAULT_RETURN_TO):
    ...
    key_facts = {
        ...
        "active_business_units": ", ".join(base["summary"].get("active_business_unit_values") or []) or "-",
...
def vendor_ownership_page(request: Request, vendor_id: str, return_to: str = VENDOR_DEFAULT_RETURN_TO):
    ...
    ownership_active_business_units: list[str] = []
    for row in org_assignments_rows:
        ...
        if business_unit_value and business_unit_value not in ownership_active_business_units:
            ownership_active_business_units.append(business_unit_value)
    if current_owner_org_id and current_owner_org_id not in ownership_active_business_units:
        ownership_active_business_units.insert(0, current_owner_org_id)
    ownership_business_unit_items: list[dict[str, str | bool]] = []
    for business_unit_value in ownership_active_business_units:
        ownership_business_unit_items.append(
            {
                "business_unit": business_unit_value,
                "is_primary": bool(current_owner_org_id and business_unit_value == current_owner_org_id),
            }
        )
    ownership_business_unit_items.sort(
        key=lambda row: (not bool(row.get("is_primary")), str(row.get("business_unit") or "").lower())
    )
    ...
    context = base_template_context(
        ...
        extra={
            ...
            "ownership_active_business_units": ownership_active_business_units,
            "ownership_business_unit_items": ownership_business_unit_items,
            "ownership_business_unit_options": _offering_business_unit_options(repo),
...
@router.post("/{vendor_id}/ownership/business_unit/update")
@require_permission("vendor_edit")
async def update_vendor_ownership_business_unit_submit(request: Request, vendor_id: str):
    repo = get_repo()
    user = get_user_context(request)
    form = await request.form()
    return_to = _safe_return_to(str(form.get("return_to", f"/vendors/{vendor_id}/ownership")))
    selected_business_units = [str(item).strip() for item in form.getlist("business_units") if str(item).strip()]
    submitted_multi_business_units = bool(selected_business_units)
    legacy_business_unit = str(form.get("business_unit", "")).strip()
    if legacy_business_unit and legacy_business_unit not in selected_business_units:
        selected_business_units.insert(0, legacy_business_unit)
    primary_business_unit = str(form.get("primary_business_unit", "")).strip()
    if not primary_business_unit and legacy_business_unit and not submitted_multi_business_units:
        primary_business_unit = legacy_business_unit
    ...
    allowed_business_units = _offering_business_unit_options(repo)
    canonical_by_lower = {str(item).lower(): str(item) for item in allowed_business_units}
    normalized_business_units: list[str] = []
    for candidate in selected_business_units:
        canonical = canonical_by_lower.get(candidate.lower())
        if canonical and canonical not in normalized_business_units:
            normalized_business_units.append(canonical)
    normalized_primary_business_unit = ""
    if primary_business_unit:
        normalized_primary_business_unit = canonical_by_lower.get(primary_business_unit.lower(), "")
        if not normalized_primary_business_unit:
            add_flash(request, "Primary business unit must be selected from the admin-managed list.", "error")
            return RedirectResponse(url=return_to, status_code=303)
        if normalized_primary_business_unit not in normalized_business_units:
            add_flash(request, "Primary business unit must be one of the selected business units.", "error")
            return RedirectResponse(url=return_to, status_code=303)
    if not normalized_business_units and normalized_primary_business_unit:
        add_flash(request, "Primary business unit must be one of the selected business units.", "error")
        return RedirectResponse(url=return_to, status_code=303)

    try:
        payload_updates = {"owner_org_id": normalized_primary_business_unit or ""}
        if user.can_apply_change("update_vendor_profile"):
            ...
            existing_business_units: set[str] = set()
            if not existing_assignments.empty:
                ...
                existing_business_units = {str(item).strip() for item in active_rows["org_id"].tolist() if str(item).strip()}
            for business_unit_value in normalized_business_units:
                if business_unit_value in existing_business_units:
                    continue
                repo.add_vendor_org_assignment(
                    vendor_id=vendor_id,
                    org_id=business_unit_value,
                    assignment_type="consumer",
                    actor_user_principal=user.user_principal,
                )
            add_flash(
                request,
                f"Business Unit updated. Request ID: {result['request_id']} | Audit Event: {result['change_event_id']}",
                "success",
            )
        else:
            request_id = repo.create_vendor_change_request(
                ...
                payload={"updates": payload_updates, "business_units": normalized_business_units, "reason": reason},
            )
            add_flash(request, f"Pending change request submitted: {request_id}", "success")
        repo.log_usage_event(
            user_principal=user.user_principal,
            page_name="vendor_ownership",
            event_type="update_vendor_ownership_business_unit",
            payload={
                "vendor_id": vendor_id,
                "business_unit": normalized_primary_business_unit,
        ...
        add_flash(request, f"Could not update Business Unit: {exc}", "error")
    return RedirectResponse(url=return_to, status_code=303)
...
@router.post("/{vendor_id}/business_unit-assignments/add")
...
async def add_vendor_org_assignment_submit(request: Request, vendor_id: str):
    ...
    try:
        ...
        if user.can_apply_change("add_vendor_org_assignment"):
            ...
            add_flash(request, f"Business Unit assignment added: {assignment_id}", "success")
        else:
            request_id = repo.create_vendor_change_request(
                vendor_id=vendor_id,
                requestor_user_principal=user.user_principal,
        ...
        add_flash(request, f"Could not add Business Unit assignment: {exc}", "error")
    return RedirectResponse(url=return_to, status_code=303)
