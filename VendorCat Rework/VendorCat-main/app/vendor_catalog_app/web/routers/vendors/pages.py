...
from vendor_catalog_app.web.routers.vendors.constants import (
    ...
    LIFECYCLE_STATES,
    OFFERING_BUSINESS_UNIT_FALLBACK,
...
def _offering_business_unit_options(repo) -> list[str]:
    options = [str(item).strip() for item in repo.list_offering_business_unit_options() if str(item).strip()]
    return options or list(OFFERING_BUSINESS_UNIT_FALLBACK)
...
def _normalize_offering_business_unit(
    repo, value: str, *, allow_blank: bool = True, extra_allowed: set[str] | None = None
) -> str:
    allowed = _offering_business_unit_options(repo)
    business_unit = (value or "").strip()
    if not business_unit:
        return "" if allow_blank else allowed[0]
    canonical_by_lower = {item.lower(): item for item in allowed}
    canonical = canonical_by_lower.get(business_unit.lower())
    if canonical:
        return canonical
    if extra_allowed and business_unit in extra_allowed:
