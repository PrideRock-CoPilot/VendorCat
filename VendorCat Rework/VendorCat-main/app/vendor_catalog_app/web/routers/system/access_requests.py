...
async def submit_access_request(request: Request):
    ...
    if active_roles and not user.can_apply_change("request_access"):
        add_flash(request, "You do not have permission to submit this request.", "error")
...
async def bootstrap_initial_admin(request: Request):
    ...
    active_roles = set(getattr(user, "roles", set()) or set())
    if active_roles and not user.can_apply_change("feedback_submit"):
        add_flash(request, "You do not have permission to run admin bootstrap.", "error")
        return RedirectResponse(url="/dashboard", status_code=303)
    form = await request.form()
...
async def accept_terms(request: Request):
    ...
    active_roles = set(getattr(user, "roles", set()) or set())
    if active_roles and not user.can_apply_change("request_access"):
        add_flash(request, "You do not have permission to accept terms in this context.", "error")
