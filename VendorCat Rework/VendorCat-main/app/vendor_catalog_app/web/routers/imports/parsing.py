...
def _resolve_xml_record_elements(
    ...
    *,
    ...
    best_structured_nodes: list[ET.Element] = []
    best_structured_selector = ""
    best_structured_score: tuple[int, int, int] | None = None
    best_leaf_nodes: list[ET.Element] = []
    best_leaf_selector = ""
    best_leaf_depth = 10**6
    best_leaf_count = 0
    ...
    while queue:
        ...
        for child_name, count in child_counts.items():
            ...
            nested_child_count = sum(len(list(node)) for node in matched)
            if nested_child_count > 0:
                score = (depth, -nested_child_count, -len(matched))
                if best_structured_score is None or score < best_structured_score:
                    best_structured_nodes = matched
                    best_structured_selector = selector
                    best_structured_score = score
                continue
            if len(matched) > best_leaf_count or (len(matched) == best_leaf_count and depth < best_leaf_depth):
                best_leaf_nodes = matched
                best_leaf_selector = selector
                best_leaf_depth = depth
                best_leaf_count = len(matched)
        for child in children:
            queue.append((child, path_tokens_active + [_xml_local_name(child.tag)], depth + 1))

    if best_structured_nodes:
        return best_structured_nodes, best_structured_selector
    if best_leaf_nodes:
        return best_leaf_nodes, best_leaf_selector
...
def _mapping_candidate_score(target_field: str, source_key: str) -> int:
    alias_map = {
        ...
        "supplier_number": "vendor_id",
        "supplier_num": "vendor_id",
        "vendor_name": "legal_name",
        "vendor_category_name": "vendor_category",
        "compliance_cat": "compliance_category",
        "gl_cat": "gl_category",
        "delegated_vendor": "delegated_vendor_flag",
        "healthcare_vendor": "health_care_vendor_flag",
        "health_care_vendor": "health_care_vendor_flag",
    ...
    # Prefer contact-role mapping when source headers encode repeated contact fields
    # like contact_1_name/contact_2_email in flat spreadsheet uploads.
    if "contact" in source_match:
        if target_norm == "full_name" and source_match.endswith("_name"):
            return 96
        if target_norm == "email" and source_match.endswith("_email"):
            return 96
        if target_norm == "phone" and (source_match.endswith("_phone") or source_match.endswith("_tel")):
            return 96
        if target_norm == "contact_type" and source_match.endswith("_type"):
            return 96
...
def _source_group_key(source_key: str) -> tuple[str, bool]:
    ...
    if len(parts) <= 1:
        flat_key = normalize_column_name(key)
        flat_match = re.match(
            r"^(?P<prefix>[a-z0-9]+(?:_[a-z0-9]+)*)_(?P<index>\d+)_(?P<field>[a-z0-9_]+)$",
            flat_key,
        )
        if flat_match:
            prefix = str(flat_match.group("prefix") or "")
            index = str(flat_match.group("index") or "")
            if "contact" in prefix or "owner" in prefix:
                return f"{prefix}_{index}", True
        glued_match = re.match(
            r"^(?P<prefix>[a-z0-9_]*?(?:contact|owner))(?P<index>\d+)_(?P<field>[a-z0-9_]+)$",
            flat_key,
        )
        if glued_match:
            prefix = str(glued_match.group("prefix") or "")
            index = str(glued_match.group("index") or "")
            return f"{prefix}_{index}", True
        return "__static__", False
...
def render_context(
    *,
    ...
    import_reason: str = "",
    show_guided_tour: bool = False,
    imports_guided_ux_v3_enabled: bool = True,
) -> dict[str, Any]:
    ...
    selected_profile_id = str(selected_mapping_profile_id or "").strip()
    for profile in list(mapping_profiles or []):
    ...
    for row in list(preview_rows or []):
        ...
        if status in {"error", "blocked", "review"}:
            problem_counts[status] += 1
        else:
            problem_counts["ready"] += 1
    guided_problem_counts = {
        "error": int(problem_counts.get("error", 0)),
        "blocked": int(problem_counts.get("blocked", 0)),
        "review": int(problem_counts.get("review", 0)),
        "ready": int(problem_counts.get("ready", 0)),
        "total": int(len(preview_rows or [])),
    }

    selected_profile_compatible = True
    if isinstance(selected_profile, dict) and "compatible" in selected_profile:
        selected_profile_compatible = bool(selected_profile.get("compatible"))

    guided_confidence_level = "medium"
    if guided_problem_counts["error"] > 0 or required_remaining_count > 0:
        guided_confidence_level = "low"
    elif guided_problem_counts["blocked"] > 0 or guided_problem_counts["review"] > 0:
        guided_confidence_level = "medium"
    elif bool(preview_rows):
        guided_confidence_level = "high"

    guided_confidence_reasons: list[str] = []
    if required_remaining_count > 0:
        guided_confidence_reasons.append(
            f"{required_remaining_count} required destination field(s) are still unmapped."
        )
    if guided_problem_counts["error"] > 0:
        guided_confidence_reasons.append(
            f"{guided_problem_counts['error']} row(s) have blocking validation errors."
        )
    if guided_problem_counts["blocked"] > 0:
        guided_confidence_reasons.append(
            f"{guided_problem_counts['blocked']} row(s) are blocked by dependency order."
        )
    if guided_problem_counts["review"] > 0:
        guided_confidence_reasons.append(
            f"{guided_problem_counts['review']} row(s) need review before apply."
        )
    if selected_profile_id and not selected_profile_compatible:
        guided_confidence_reasons.append(
            "Selected mapping profile does not fully match this file signature."
        )
    if not selected_profile_id and bool(preview_rows) and guided_confidence_level != "high":
        guided_confidence_reasons.append(
            "No mapping profile is selected for this file."
        )
    if not guided_confidence_reasons and bool(preview_rows):
        guided_confidence_reasons.append("Required mappings are complete and no blocking issues were found.")

    guided_requires_profile_confirmation = bool(
        bool(preview_rows)
        and guided_confidence_level == "low"
        and (not selected_profile_id or not selected_profile_compatible)
    )
    ...
    return {
        ...
        "guided_confidence_level": guided_confidence_level,
        "guided_confidence_reasons": guided_confidence_reasons,
        "guided_requires_profile_confirmation": guided_requires_profile_confirmation,
        "guided_problem_counts": guided_problem_counts,
        "show_guided_tour": bool(show_guided_tour),
        "imports_guided_ux_v3_enabled": bool(imports_guided_ux_v3_enabled),
    }
