...
def _parse_bool_flag(value: Any) -> int | None:
    cleaned = str(value or "").strip().lower()
    if not cleaned:
        return None
    if cleaned in {"1", "true", "yes", "y", "x", "checked"}:
        return 1
    if cleaned in {"0", "false", "no", "n", "unchecked"}:
        return 0
    return None


def _extra_vendor_updates_from_row(row_data: dict[str, str]) -> dict[str, Any]:
    updates: dict[str, Any] = {}
    for field in ("vendor_category", "compliance_category", "gl_category"):
        value = str(row_data.get(field) or "").strip()
        if value:
            updates[field] = value

    delegated_flag = _parse_bool_flag(row_data.get("delegated_vendor_flag"))
    if delegated_flag is not None:
        updates["delegated_vendor_flag"] = delegated_flag

    health_care_flag = _parse_bool_flag(row_data.get("health_care_vendor_flag"))
    if health_care_flag is not None:
        updates["health_care_vendor_flag"] = health_care_flag
    return updates
...
def _vendor_updates_from_row(row_data: dict[str, str]) -> dict[str, Any]:
    ...
    for field in ("legal_name", "display_name", "lifecycle_state", "owner_org_id", "risk_tier"):
        ...
        if value:
            updates[field] = value
    updates.update(_extra_vendor_updates_from_row(row_data))
    return updates
    ...
    *,
    ...
    if layout_key == "vendors":
        if selected_action == "new":
            ...
            metadata_updates = _extra_vendor_updates_from_row(row_data)
            if metadata_updates and hasattr(repo, "apply_vendor_profile_update"):
                try:
                    repo.apply_vendor_profile_update(
                        vendor_id=str(created_vendor_id or "").strip(),
                        actor_user_principal=actor_user_principal,
                        updates=metadata_updates,
                        reason=reason or "bulk import metadata",
                    )
                except Exception:
                    pass
            notes_text = str(row_data.get("notes") or "").strip()
            if notes_text and hasattr(repo, "create_app_note"):
                try:
                    repo.create_app_note(
                        entity_name="vendor",
                        entity_id=str(created_vendor_id or "").strip(),
                        note_type="import_note",
    ...
    if layout_key == "offerings":
        if selected_action == "new":
            ...
            created_offering_id = repo.create_offering(
                ...
                business_unit=str(row_data.get("business_unit") or "").strip() or None,
    ...
    *,
    ...
    for area_row in area_rows_by_area.get("offering", []):
        ...
        created_offering_id = repo.create_offering(
            ...
            business_unit=_payload_value(payload, "business_unit") or None,
    ...
    for area_row in area_rows_by_area.get("vendor_identifier", []):
        payload = dict(area_row.get("payload") or {})
        vendor_id = resolve_vendor_id(payload=payload, offering_id="")
        if not vendor_id or not hasattr(repo, "upsert_vendor_identifier"):
            continue
        identifier_type = _payload_value(payload, "identifier_type")
        identifier_value = _payload_value(payload, "identifier_value")
        ...
        try:
            identifier_id = repo.upsert_vendor_identifier(
                ...
                is_primary=str(_payload_value(payload, "is_primary")).strip().lower() in {"1", "true", "yes", "y"},
                country_code=_payload_value(payload, "country_code") or None,
                actor_user_principal=actor_user_principal,
            )
        except Exception:
            continue
        if str(identifier_id or "").strip():
            child_counts["vendor_identifier"] = child_counts.get("vendor_identifier", 0) + 1
