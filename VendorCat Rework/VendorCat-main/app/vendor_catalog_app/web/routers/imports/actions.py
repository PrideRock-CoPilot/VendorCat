...
from vendor_catalog_app.web.routers.imports.guided import (
    dismiss_imports_guided_tour,
    imports_guided_enabled,
    log_imports_guided_event,
    show_imports_guided_tour,
)
...
def _guided_render_args(repo, user) -> dict[str, Any]:
    guided_enabled = imports_guided_enabled(user.config)
    return {
        "show_guided_tour": show_imports_guided_tour(
            repo,
            user_principal=user.user_principal,
            guided_enabled=guided_enabled,
        ),
        "imports_guided_ux_v3_enabled": guided_enabled,
    }
...
def _pending_lookup_candidate_count(repo, *, import_job_id: str) -> int:
    job_id = str(import_job_id or "").strip()
    if not job_id or not hasattr(repo, "count_import_lookup_candidates"):
        return 0
    try:
        return int(repo.count_import_lookup_candidates(import_job_id=job_id, status="pending") or 0)
    except Exception:
        return 0


def _pending_lookup_candidate_rows(repo, *, import_job_id: str) -> list[dict[str, Any]]:
    job_id = str(import_job_id or "").strip()
    if not job_id or not hasattr(repo, "list_import_lookup_candidates"):
        return []
    try:
        return list(repo.list_import_lookup_candidates(import_job_id=job_id, status="pending") or [])
    except Exception:
        return []


def _merge_primary_stage_payload(
    *,
    layout_key: str,
    row_index: int,
    ...
    merged: dict[str, str] = {str(k): str(v or "").strip() for k, v in dict(row_data or {}).items()}
    primary_area = str(LAYOUT_PRIMARY_STAGE_AREA.get(str(layout_key or "").strip().lower()) or "").strip()
    if not primary_area:
        return merged
    for stage_row in list(dict(stage_area_rows or {}).get(primary_area, []) or []):
        if int(stage_row.get("row_index") or 0) != int(row_index or 0):
            continue
        payload = dict(stage_row.get("payload") or {})
...
async def imports_preview(request: Request):
    ...
    guided_render = _guided_render_args(repo, user)
    form = await request.form()
    ...
    if len(uploads) == 1:
        ...
        log_imports_guided_event(
            repo,
            user_principal=user.user_principal,
            event_type="imports_guided_step_view",
            payload={"step": "fix_matches", "bundle_mode": False, "rows": preview_total_rows},
        )
    ...
    log_imports_guided_event(
        ...
        user_principal=user.user_principal,
        event_type="imports_guided_step_view",
        payload={"step": "fix_matches", "bundle_mode": True, "rows": preview_total_rows},
    )
    return request.app.state.templates.TemplateResponse(request, "imports.html", context)
...
async def imports_apply(request: Request):
    ...
    if str(form.get("guided_profile_continue_confirm", "")).strip():
        log_imports_guided_event(
            repo,
            user_principal=user.user_principal,
            event_type="imports_guided_low_confidence_continue",
            payload={"apply_mode": apply_mode},
        )
    if str(form.get("guided_final_confirm", "")).strip():
        log_imports_guided_event(
            repo,
            user_principal=user.user_principal,
            event_type="imports_guided_apply_confirmed",
            payload={"apply_mode": apply_mode},
        )
    payload = load_preview_payload(preview_token)
    if payload is None:
    ...
    if bool(payload.get("is_bundle")):
        ...
        if apply_mode != "stage_only":
            ...
            for file_payload in bundle_files_payload:
                import_job_id = str(file_payload.get("import_job_id") or "").strip()
                pending_count = _pending_lookup_candidate_count(repo, import_job_id=import_job_id)
                if pending_count <= 0:
                    continue
                file_name = str(file_payload.get("file_name") or "").strip() or "bundle file"
                pending_items.append(f"{file_name}: {pending_count} pending lookup candidate(s)")
            if pending_items:
                add_flash(
                    request,
                    "Import apply blocked: pending governed lookup candidates require steward review.",
                    "error",
                )
                add_flash(request, " | ".join(pending_items), "info")
                return RedirectResponse(url="/imports", status_code=303)
        ...
        if apply_mode == "stage_only":
            ...
            for file_index, file_payload in ordered_files:
                ...
                for preview_row in file_rows:
                    ...
                    row_data = _merge_primary_stage_payload(
        ...
        log_imports_guided_event(
            ...
            user_principal=user.user_principal,
            event_type="imports_guided_step_view",
            payload={"step": "stage_import_complete", "bundle_mode": True},
        )
        return request.app.state.templates.TemplateResponse(request, "imports.html", context)
    ...
    pending_lookup_count = _pending_lookup_candidate_count(repo, import_job_id=import_job_id)
    if pending_lookup_count > 0:
        add_flash(
            request,
            (
                "Import apply blocked: pending governed lookup candidates require steward review "
                f"({pending_lookup_count} pending)."
            ),
            "error",
        )
        return RedirectResponse(url="/imports", status_code=303)
    ...
    log_imports_guided_event(
        ...
        user_principal=user.user_principal,
        event_type="imports_guided_step_view",
        payload={"step": "stage_import_complete", "bundle_mode": False},
    )
    return request.app.state.templates.TemplateResponse(request, "imports.html", context)


@router.get("/imports/lookup-candidates/{import_job_id}")
@require_permission("import_preview")
async def import_lookup_candidates(request: Request, import_job_id: str, status: str = "pending"):
    imports_module = _imports_module()
    repo = imports_module.get_repo()
    user = imports_module.get_user_context(request)
    imports_module.ensure_session_started(request, user)
    ...
    if not can_manage_imports(user):
        return JSONResponse({"error": "You do not have permission to view import lookup candidates."}, status_code=403)
    if not hasattr(repo, "list_import_lookup_candidates"):
        return JSONResponse({"items": [], "pending_count": 0})
    ...
    items = list(
        repo.list_import_lookup_candidates(
            import_job_id=str(import_job_id or "").strip(),
            status=normalized_status,
        )
        or []
    )
    pending_count = len([row for row in items if str(row.get("status") or "").strip().lower() == "pending"])
    return JSONResponse(
        {
            "import_job_id": str(import_job_id or "").strip(),
            "status": normalized_status,
...
@router.post("/imports/lookup-candidates/{candidate_id}/review")
@require_permission("import_apply")
async def review_import_lookup_candidate_submit(request: Request, candidate_id: str):
    imports_module = _imports_module()
    repo = imports_module.get_repo()
    user = imports_module.get_user_context(request)
    imports_module.ensure_session_started(request, user)
    ...
    if not can_manage_imports(user):
        return JSONResponse({"error": "You do not have permission to review import lookup candidates."}, status_code=403)
    if write_blocked(user):
        return JSONResponse({"error": "Application is in locked mode. Import actions are disabled."}, status_code=423)
    if not hasattr(repo, "review_import_lookup_candidate"):
        return JSONResponse({"error": "Lookup candidate review is not available in this runtime."}, status_code=501)

    payload = await _event_payload(request)
    decision = str(payload.get("decision") or "pending").strip().lower()
    if decision not in {"pending", "approved", "rejected"}:
        return JSONResponse({"error": "Decision must be one of: pending, approved, rejected."}, status_code=400)
    review_note = str(payload.get("review_note") or payload.get("note") or "").strip()

    updated = repo.review_import_lookup_candidate(
        candidate_id=str(candidate_id or "").strip(),
        decision=decision,
        review_note=review_note,
        actor_user_principal=user.user_principal,
        create_lookup_option_on_approve=True,
    )
    if not updated:
        return JSONResponse({"error": "Lookup candidate not found."}, status_code=404)
