(() => {
  ...
  const initSortableFilterTable = ({
    ...
    const updateIndicators = () => {
      Object.keys(indicatorByKey).forEach((key) => {
        ...
        node.textContent = state.dir === "asc" ? "â–²" : "â–¼";
      });
    };
  ...
  const initUserDrawer = () => {
    ...
    const businessUnitActionButtons = Array.from(document.querySelectorAll("[data-business_unit-action]"));
    const businessUnitCheckboxes = Array.from(document.querySelectorAll("input[data-business_unit-checkbox]"));
    ...
    const setSelectedBusinessUnits = (businessUnitValues) => {
      const selected = new Set((businessUnitValues || []).map((item) => String(item || "").trim()));
      businessUnitCheckboxes.forEach((checkbox) => {
    ...
    const setAddMode = () => {
      ...
      if (subtitle) {
        subtitle.textContent = "Select one user, one role, then apply Business Unit access in one save.";
      }
      updateLoginPreview();
    };
    ...
    const setEditMode = (button) => {
      ...
      const businessUnitCsv = String(button.getAttribute("data-target-business_units") || "").trim();
      const businessUnitValues = businessUnitCsv ? businessUnitCsv.split(",").map((item) => item.trim()).filter((item) => item) : [];
      ...
      setSelectedBusinessUnits(businessUnitValues);
      setUserLocked(true);
      if (title) {
    ...
    businessUnitActionButtons.forEach((button) => {
      button.addEventListener("click", () => {
        const action = String(button.getAttribute("data-business_unit-action") || "").trim().toLowerCase();
        if (action === "all") {
          businessUnitCheckboxes.forEach((checkbox) => {
