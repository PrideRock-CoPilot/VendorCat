...
class RepositoryReportingVendorsMixin:
    def list_vendors_page(
        ...
        *,
        ...
        if not bool(include_merged):
            where_parts.append("(v.merged_into_vendor_id IS NULL OR trim(coalesce(v.merged_into_vendor_id, '')) = '')")
        if search_text.strip():
            like = f"%{search_text.strip()}%"
            where_parts.append(
                self._sql(
                    "reporting/filter_vendors_page_search_clause.sql",
                    core_vendor_offering=self._report_table("core_vendor_offering"),
                    core_contract=self._report_table("core_contract"),
                    core_vendor_business_owner=self._report_table("core_vendor_business_owner"),
                    core_offering_business_owner=self._report_table("core_offering_business_owner"),
                    core_vendor_contact=self._report_table("core_vendor_contact"),
                    core_offering_contact=self._report_table("core_offering_contact"),
                    core_vendor_demo=self._report_table("core_vendor_demo"),
    ...
    def get_vendor_identifiers(self, vendor_id: str) -> pd.DataFrame:
        return self._query_file(
            ...
            core_vendor_identifier=self._report_table("core_vendor_identifier"),
        )
    ...
    def get_vendor_org_assignments(self, vendor_id: str) -> pd.DataFrame:
        return self._query_file(
            ...
            core_vendor_org_assignment=self._report_table("core_vendor_org_assignment"),
        )
    ...
    def get_vendor_contract_events(self, vendor_id: str) -> pd.DataFrame:
        out = self._query_file(
            ...
            core_contract_event=self._report_table("core_contract_event"),
            core_contract=self._report_table("core_contract"),
        )
    ...
    def get_vendor_demo_scores(self, vendor_id: str) -> pd.DataFrame:
        return self._query_file(
            ...
            core_vendor_demo_score=self._report_table("core_vendor_demo_score"),
            core_vendor_demo=self._report_table("core_vendor_demo"),
        )
    ...
    def get_vendor_demo_notes(self, vendor_id: str) -> pd.DataFrame:
        return self._query_file(
            ...
            core_vendor_demo_note=self._report_table("core_vendor_demo_note"),
            core_vendor_demo=self._report_table("core_vendor_demo"),
        )
    ...
    def vendor_summary(self, vendor_id: str, months: int = 12) -> dict[str, Any]:
        ...
        if "lifecycle_state" in active_offerings.columns:
            ...
            # Fall back to all offerings so summary cards still show useful business-unit/service values.
            if active_offerings.empty:
                active_offerings = offerings.copy()
        active_business_unit_values: list[str] = []
        active_service_type_values: list[str] = []
        if not active_offerings.empty:
            if "business_unit" in active_offerings.columns:
                active_business_unit_values = sorted(
                    {
                        str(value).strip()
                        for value in active_offerings["business_unit"].tolist()
                        if str(value).strip()
                    }
                )
        ...
        return {
            ...
            "active_business_unit_values": active_business_unit_values,
