...
class RepositoryOfferingWriteMixin:
    ...
    def create_offering(
        ...
        *,
        ...
        self._canonical_ensure_vendor_from_core(vendor_id, actor_user_principal=actor_user_principal)
    ...
    def update_offering_fields(
        ...
        *,
        ...
        request_id = str(uuid.uuid4())
        legacy_updates = {k: v for k, v in clean_updates.items() if k != "business_unit_ids"}
        set_clause = ", ".join([f"{k} = %s" for k in legacy_updates])
        params = list(legacy_updates.values()) + [offering_id, vendor_id]
        self._ensure_local_offering_columns()
        if set_clause:
            self._execute_file(
        ...
        self._canonical_upsert_offering(
            offering_id=offering_id,
            vendor_id=vendor_id,
            offering_name=str(after.get("offering_name") or offering_id),
            lifecycle_state=after.get("lifecycle_state"),
            business_unit=after.get("business_unit"),
            service_type=after.get("service_type"),
            criticality_tier=after.get("criticality_tier"),
            updated_by=actor_user_principal,
        ...
        incoming_business_units = clean_updates.get("business_unit_ids")
        if isinstance(incoming_business_units, list):
            normalized_values = [str(item or "").strip() for item in incoming_business_units]
            for business_unit_value in normalized_values:
                ...
                self._canonical_upsert_offering_business_unit_assignment(
                    ...
                    source_system="runtime_update",
                    source_key=f"update_offering_fields:{offering_id}",
                    is_primary=bool(
                        str(after.get("business_unit") or "").strip().lower() == business_unit_value.lower()
                    ),
                    active_flag=True,
                    actor_user_principal=actor_user_principal,
                )
    ...
    def add_vendor_owner(
        ...
        *,
        ...
        self._canonical_upsert_vendor_owner_assignment(
            ...
            vendor_id=vendor_id,
            owner_user_principal=row["owner_user_principal"],
    ...
    def remove_offering_owner(
        ...
        *,
        ...
        self._canonical_deactivate_offering_owner_assignment(
            assignment_id=str(offering_owner_id),
    ...
    def bulk_reassign_offering_owner(
        ...
        *,
        ...
        for row in rows_to_update.to_dict("records"):
            ...
            self._canonical_reassign_offering_owner_assignment(
                assignment_id=owner_id,
                owner_user_principal=to_owner_ref,
            )
    ...
    def remove_offering_contact(
        ...
        *,
        ...
        self._canonical_deactivate_offering_contact(
            contact_id=str(offering_contact_id),
            offering_id=str(offering_id),
        )
        self._write_audit_entity_change(
            entity_name="core_offering_contact",
