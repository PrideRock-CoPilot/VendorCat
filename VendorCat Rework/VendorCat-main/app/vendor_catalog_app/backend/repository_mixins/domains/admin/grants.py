...
class RepositoryAdminGrantMixin:
    ...
    def list_scope_grants(
        ...
        *,
        ...
        """List Business Unit scope grants assigned to users."""
        user_filter, _user_like = self._normalized_filter(user_principal)
        ...
        *,
        ...
        for item in assignments:
            ...
            if assignment_type == "vendor_owner":
                ...
                self._canonical_reassign_vendor_owner_assignment(
                    assignment_id=assignment_id,
                    owner_user_principal=target_ref,
                )
    ...
    def grant_org_scope(
        ...
        """Grant Business Unit scope to a user and write an access audit event."""
        target_ref = self.resolve_user_id(target_user_principal, allow_create=True)
        granted_by_ref = self.resolve_user_id(granted_by, allow_create=True)
        if not target_ref or not granted_by_ref:
            raise ValueError("Target user and grant actor must resolve to directory users.")
        ...
        self._audit_access(
            ...
            target_user_principal=target_ref,
            target_role=None,
            notes=f"Business Unit scope granted: {org_id} ({scope_level}).",
        )
    ...
    def revoke_org_scope(
        ...
        *,
        ...
        if not org_key or not scope_key:
            raise ValueError("Business Unit and scope level are required.")
        ...
        self._audit_access(
            ...
            target_user_principal=target_ref,
            target_role=None,
            notes=f"Business Unit scope revoked: {org_key} ({scope_key}).",
        )
