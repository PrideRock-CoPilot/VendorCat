...
async def save_user_access(request: Request):
    ...
    raw_lob_ids = list(form.getlist("lob_ids"))
    ...
    desired_lobs = sorted(
        {
            str(item).strip()
            for item in raw_lob_ids
            if str(item).strip() and str(item).strip().lower() != "all"
        }
    )
    desired_scope_orgs = set(desired_lobs) if scope_level != "none" else set()
    ...
    try:
        ...
        if scope_changes_requested and not user.can_apply_change("admin_scope_manage"):
            add_flash(request, "You do not have permission to manage Line of Business access.", "error")
            return RedirectResponse(url=_access_tab_redirect(selected_tab), status_code=303)
    ...
    repo.log_usage_event(
        ...
        payload={
            ...
            "scope_level": scope_level,
            "lob_count": len(desired_scope_orgs),
    ...
    add_flash(
        ...
        (
            ...
            f"({len(desired_scope_orgs)} LOBs selected, +{granted_scopes} / -{revoked_scopes} scope changes)."
        ),
        "success",
    )
    return RedirectResponse(url=_access_tab_redirect(selected_tab), status_code=303)
