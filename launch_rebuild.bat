...
if /I "%CLEAN_DB%"=="true" (
  echo Running clean rebuild migration...
...
echo Starting VendorCatalog rebuild...
...
if errorlevel 1 (
  echo.
  echo Rebuild app failed to start.
...
:usage
echo Usage:
echo   launch_rebuild.bat [host] [port] [--clean-db] [--skip-migrate] [--persona ^<name^>] [--select-user] [--no-persona-prompt]
echo.
echo Persona values:
echo   admin ^| reviewer ^| editor ^| viewer ^| watcher ^| anonymous
echo.
echo Examples:
echo   launch_rebuild.bat
  
echo   launch_rebuild.bat 127.0.0.1 8010
  
echo   launch_rebuild.bat 0.0.0.0 8010 --clean-db
  
echo   launch_rebuild.bat 0.0.0.0 8010 --skip-migrate
  
echo   launch_rebuild.bat --persona admin
  
echo   launch_rebuild.bat --select-user
exit /b 0
